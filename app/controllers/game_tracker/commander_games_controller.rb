module GameTracker
  class CommanderGamesController < BaseController
    before_action :authenticate_user!, only: %i[new create edit update destroy search_opponents]
    ...
    def index
      ...
      games = tracker_owner.commander_games
    ...
    def create
      ...
      if @commander_game.save
        redirect_to game_tracker_commander_game_path(current_user.username, @commander_game), notice: 'Game recorded!'
      else
        @tracked_decks = current_user.tracked_decks.not_retired.includes(:commander)
    ...
    def destroy
      ...
      redirect_to game_tracker_commander_games_path(current_user.username), notice: 'Game removed.', status: :see_other
    end
    ...
    def ensure_owner
      return if logged_in? && @commander_game.user_id == current_user.id
